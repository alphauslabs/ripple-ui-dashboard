...
>(({ className, children, label, required, ...props }, ref) => (
  <div className={'flex flex-col gap-2 ' + className}>
    {label && (
      <label className="text-sm font-medium leading-4 tracking-tight text-left gap-1">
        {label}
...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        'relative z-50 max-h-96 overflow-hidden rounded-md border border-gray-300 bg-white p-[8px] shadow-sm',
        'w-[var(--radix-select-trigger-width)]', // Match trigger width
        position === 'popper' &&
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn('py-2 px-3 text-sm font-medium text-[#161B29]', className)}
    {...props}
  />
));
