...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List
    ref={ref}
    className={cn(
      // 'border-b-[1px] border-black w-full flex justify-start',
      'border-b-[1px] border-[#DFE2F6] w-full flex justify-start', //changed border line to neutral color
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Trigger
    ...
    className={cn(
      ...
      // Apply the border bottom for active state
      'data-[state=active]:border-b-2 data-[state=active]:border-blue-600 data-[state=active]:text-blue-600',
      // Ensure non-active tabs have no border and appropriate text color
