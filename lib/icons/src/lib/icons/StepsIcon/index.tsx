...
interface StepsIconProps extends React.HTMLAttributes<HTMLDivElement> {
  variant?: 'checked' | 'numbered-active' | 'numbered-inactive';
  number?: number;
}

const StepsIcon = forwardRef<HTMLDivElement, StepsIconProps>(
  ({ variant = 'numbered-inactive', number = 0, className }) => {
    if (variant === 'checked') {
      return (
        <svg
          width="24"
        ...
        >
          <g clipPath="url(#clip0_1003_7042)">
            <path
    ...
    const renderedStyle =
      variant === 'numbered-active'
        ? 'bg-[#1D55CE] text-white'
        : 'bg-[#DFE2F6] text-[#161B29]';

    return (
      <div
        className={`${renderedStyle} flex items-center justify-center w-6 h-6 rounded-full border ${className}`}
      >
